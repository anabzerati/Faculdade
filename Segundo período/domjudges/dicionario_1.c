...
ListaPalavras *inserir(ListaPalavras*h, char*palavraadicionar){
    ...
    while(aux->prox && strcmp(aux->palavra,nova->palavra)<0){ //percorre a lista até achar onde colocar
        prev=aux;
        aux=aux->prox;
    }

    if(strcmp(aux->palavra,nova->palavra)==0){ //se já tiver a palavra, fazer nada 
        free(nova);
        return (h);
    }

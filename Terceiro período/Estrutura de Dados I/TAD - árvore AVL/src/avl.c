...
no *remover(no *raiz, int chave){
    ...
    if(raiz->chave == chave){ //encontrou
        if(!raiz->esq && !raiz->dir){
            ...
            raiz->chave = aux->chave; //troca raiz e o maior dentre os elementos menores que a raiz
            aux->chave = chave;

            raiz->esq = remover(raiz->esq, chave); //vai percorrer os filhos a esquerda da raiz (novo valor) até encontrar a chave (antigo valor da raíz), que é nó folha
            return raiz;
        } else{
            no *aux;
